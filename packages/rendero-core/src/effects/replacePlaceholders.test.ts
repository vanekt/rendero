...
describe("replacePlaceholders function", () => {
  ...
  test("does not modify the input if no placeholders", () => {
    expect(replacePlaceholders({ title: "Hello, {{name}}!" }, {})).toEqual({
      title: "Hello, {{name}}!",
    });
  });
      ...
      replacePlaceholders(
        {
          object: {
            nestedObject: {
              ...
              stringWithCalc: "Total: {{e! return 1 + 5; }}",
              boolean: "{{disabled}}",
              nestedObject: {
                float: "{{opacity}}",
                bigint: "{{bigint}}",
  ...
  test("should not be eager", () => {
    expect(
      replacePlaceholders("Color: {{color}}, zIndex: {{zIndex}}", placeholders),
    ).toBe("Color: red, zIndex: 10");
  });
  ...
  test("returns original value if input is not string and not array and not object", () => {
    expect(replacePlaceholders(undefined, placeholders)).toBe(undefined);
    expect(replacePlaceholders(null, placeholders)).toBe(null);
    expect(replacePlaceholders(1, placeholders)).toBe(1);
    expect(replacePlaceholders(10n, placeholders)).toBe(10n);
    expect(replacePlaceholders(true, placeholders)).toBe(true);
    expect(replacePlaceholders(false, placeholders)).toBe(false);
    expect(replacePlaceholders(Symbol.for("foo"), placeholders)).toBe(
      Symbol.for("foo"),
    );
  });
});
